...
//    120 的区间范围为 [67.0, 120]
    ...
    /**
     * 838. 推多米诺
     * 关键：所有的牌在同一个时刻，按照dominoes中的标识方向倒下；
     * 1.最终状态:
     * <p> 原始的"R"就一定是"R"，原始的"L"就一定是"L"，只有原始状态为"."牌的倒向取决于两侧相邻的非"."的牌的倒向
     * 2.原始"."牌的倒向共有三种情况:
     * <p> 1.两侧牌的倒向相同"R...R"或"L...L"，最终状态一定为"RRRRR"或"LLLLL"
     * <p> 2.两侧牌的倒向相反"L...R"，最终状态为"L...R"
     * <p> 3.两侧牌的倒向相对"R...L"或"R....L"，最终状态为"RR.LL"或"RRRLLL"
     * <p>
     * 为了保证在区间两侧均可形成有效区间，因此:
     * <p> 区间左侧添加"L"，不影响区间内牌的倒向
     * <p> 区间右侧添加"R"，不影响区间内牌的倒向
     *
     * @param dominoes
     * @return
     */
...
//                continue;
//            }
            if (dominoes.charAt(left) == 'L' && dominoes.charAt(right) == 'L') {//区间内"."的最终状态改为"L"
                while (left <= right) {  //区间左侧和右侧均填上
                    res.append("L");
                    left++;               //此处left最终状态在right+1处，后面right++后，left和right同一起跑线
                }
                left--;//下次区间的左侧为left，即为当前的rights
            } else if (dominoes.charAt(left) == 'R' && dominoes.charAt(right) == 'R') {//区间内"."的最终状态改为"R"
                while (left <= right) {
                    res.append("R");
                    left++;
                }
                left--;
            } else if (dominoes.charAt(left) == 'L' && dominoes.charAt(right) == 'R') {//区间内"."的最终状态仍为"."
                res.append("");         //区间左侧仍为"L"，状态没变，但已经写过
                left++;
                while (left < right) {   //区间内为"."，状态没变
                    res.append(".");
                    left++;
                }
                res.append("R");         //区间右侧仍为"R"，状态没变
//                left++;
    ...
    public String pushDominoes01(String dominoes) {
        dominoes = "L" + dominoes + "R";
        int l = 0;
        StringBuilder res = new StringBuilder();
...
//        String regularWord = replace2.replaceAll(",", "");
    ...
    public String reverseOnlyLetters(String s) {
        ...
        while (left < right) {
            while (left < right && !Character.isLetter(target[left]))
                left++;
            while (left < right && !Character.isLetter(target[right]))
                right--;
    ...
    public int numRescueBoats00(int[] people, int limit) {
        Arrays.sort(people);
        int left = 0;
        int right = people.length - 1;
        int boats = 0;
        while (left < right) { //不考虑同一个人
            //只有当左侧轻的人，能和右侧重的人共同乘坐一个船的时候，left++
            if (people[right] + people[left] <= limit) {
                left++;
            }
            right--;
