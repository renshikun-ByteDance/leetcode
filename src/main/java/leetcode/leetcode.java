...
//                result="非法输入:";
    ...
    public int maxProfit2(int[] prices) {
        ...
        for (int i = 1; i < prices.length; i++) {
            if (prices[i - 1] < prices[i]) {
                sumProfile += prices[i] - prices[i - 1];
            }
        }
        return sumProfile;
    }
    ...
    public int maxProfit3(int[] prices) {
        ...
        for (int i = 1; i < prices.length; i++) {
            if (prices[i - 1] > prices[i]) {
                profiles.add(prices[i - 1] - lowerPrice);
                lowerPrice = prices[i];
            } else if (i == prices.length - 1 && prices[i] > prices[i - 1]) {
                profiles.add(prices[i] - lowerPrice);
            }
        }
        profiles.sort(Comparator.reverseOrder());
        System.out.println(profiles);
        if (profiles.size() == 1) {
            sumProfile = profiles.get(0);
        } else if (profiles.size() > 1) {
            sumProfile = profiles.get(0) + profiles.get(1);
        } else {
            return 0;
        }
        return sumProfile;
    ...
    public void rotate02(int[][] matrix) {
        ...
        for (int i = 0; i < n / 2; i++) {
            for (int j = i; j < n - i; j++) {
                ...
                matrix[n - 1 - i - j][i] = matrix[n - 1 - i - j][n - 1 - i - j];
                matrix[n - 1 - i - j][n - 1 - i - j] = matrix[i][n - 1 - i - j];
...
//            int temp = matrix[i][i + j];
//            matrix[i][i + j] = matrix[i + len - j - 1][i];
//            matrix[i + len - j - 1][i] = matrix[i + len - 1][i + len - j - 1];
