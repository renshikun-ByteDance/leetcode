...
//            if (nums[left] == nums[right]) {   //这种情况下，不需要两侧移动，单侧移动就行
    ...
    public boolean searchMatrix00(int[][] matrix, int target) {
        ...
        if (matrix[rowTemp][left] == target)    //将这个放外面，错误写法，尽量将逻辑判断和结论判断放在一起
            return true;
    ...
    public int findMin(int[] nums) {
        ...
        System.out.println("left的位置：" + left);
        System.out.println("right的位置：" + right);
        return nums[right];
    }
...
//                if ((right - left == 1 || right - left == 2) && nums[left] < nums[right])  //此时right压着最大值****
    ...
    //这种写法不必判断是否是有序序列
    public int findMin003(int[] nums) {
        int left = 0;
        int right = nums.length - 1;
        while (left < right) {
            int mid = left + ((right - left) >> 1);
            if (nums[mid] < nums[nums.length - 1])
                right = mid;                       //这样写最终状态也是 [right,left](因为while条件决定了)，不过就是移动慢点
            else if (nums[nums.length - 1] <= nums[mid])     //等号一定在此，否在上面就是死循环
                left = mid + 1;
        }
        return nums[right];
    }
    ...
    //和nums[0]比较时
    //这种写法必须判断是否是有序序列
    public int findMin002(int[] nums) {
        int left = 0;
        int right = nums.length - 1;
    ...
    //上下两种写法均可正确，但不建议采用第一种方式，因为可能会进入死循环，if中的=和下面的..=mid同时存在
    ...
    public int findPeakElement(int[] nums) {
        ...
        while (left < right) {
            int mid = left + ((right - left) >> 1);
            if (nums[mid] > nums[mid + 1])          //right右侧上山
                right = mid;
            else if (nums[mid] <= nums[mid + 1])    //left左侧上山，等号必须在此处，对比下面错误写法findPeakElement01
                //等号一定不能和直接取mid的指针一起使用，故在此
                left = mid + 1;
        }
        return right;   //因为left < right，因此最终状态一定是left == right，所以return left;等效
    }
...
//    }
     ...
     * @return
     */
    public boolean increasingTriplet(int[] nums) {    //类似双指针的算法
        int len = nums.length;
...
//        System.out.println("left: " + left);
     ...
     * 1894. 找到需要补充粉笔的学生编号
     * <p>
     * 暴力解法：这种写法在K很大时，会遍历多次chalk，时间复杂度高
     *
     * @param chalk
     * @param k
     * @return
