...
//            }
    ...
    public boolean containsNearbyAlmostDuplicate02(int[] nums, int k, int t) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            if (bucketSet.containsKey(leftBucketID) && nums[i] - bucketSet.get(leftBucketID) <= t) return true;
            if (bucketSet.containsKey(rightBucketID) && bucketSet.get(rightBucketID) - nums[i] <= t) return true;
            bucketSet.put(bucketID, (long) nums[i]);
            //维护滑动窗口的大小为k,即有k个桶，开始时（滑动窗口一直扩张），滑动窗口大小=k时，窗口滑动（左缩，右扩）
            if (i >= k)
    ...
    public long getBucketID(long num, long size) {
    ...
    public int maxScore(int[] cardPoints, int k) {
        ...
        int winsum = 0, cardsum = 0;
        for (int i = 0; i < window; i++) {
            winsum += cardPoints[i];
            cardsum += cardPoints[i];
        }
        min = winsum;
        for (int right = window; right < cardPoints.length; right++) {
            winsum += cardPoints[right];
            winsum -= cardPoints[right - window];
    ...
    public int maxScore01(int[] cardPoints, int k) {
        int window = cardPoints.length - k;
        int min = Integer.MAX_VALUE;
        int left = 0;
        int right = 0;
